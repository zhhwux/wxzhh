...
4. 整句增强：两码句（`tiger_sentence`）、二三句（`mets`）、原码句（`xumn`）   
5. 主题优化：简纯-（ui改动、键盘背景功能）
6. 交互改进：  
   - 快捷键：`Control+z`（虎单开关）/ `Control+a`（虎词开关）/ `Control+s`（虎句开关）/ `Control+y`（音码开关）/ `Control+m`（显示拆分）  
   - Tips 上屏符号改为 `/`
   - 快符引导;
   - 音码前缀'（用于音码关闭时、音码方案自造词）
   - 音码后缀`（用于音码关闭时）
...
- 4.26：简纯-主题新增配色“简明”：可在backgrounds文件夹放入“background.png”作为背景图片（或主题内更改指定名称）
